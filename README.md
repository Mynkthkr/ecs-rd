## This terraform stack can be deployed multiple times to different environments using Terraform Workspaces. So we need to export WORKSPACE and a config.yml file in specific naming convention details are below: 


## Create a new yml config file : 

``` "config-aws-infra-us-east-1-<new_env>.yml" ```



##We need export workspace before apply to create infrastructure in this format:
```export WORKSPACE<account_name>-<region>-<environment_name>```



##So export command will be for utility infra:
```export WORKSPACE aws-infra-us-east-1-utility```

##Then change directory, 

##Run following commands to initialize dependencies, plan and apply:
